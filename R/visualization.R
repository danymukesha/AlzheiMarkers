...
plot_roc_curves <- function(eval_results) {
  ggplot2::ggplot(eval_results$roc, ggplot2::aes(x = 1 - Specificity, y = Sensitivity, color = Model)) +
    ggplot2::geom_line() +
...
#'
#' @param var_imp Variable importance results from calculate_variable_importance().
...
plot_variable_importance <- function(var_imp, model_name) {
  ...
  ggplot2::ggplot(imp_df, ggplot2::aes(x = reorder(variable, Overall), y = Overall)) +
